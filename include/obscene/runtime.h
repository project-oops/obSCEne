...
/* Bootstrap the output channel for a native title, which has no payload args to
 * bootstrap from. Resolves the klog and write entry points by name through the loader's
 * own sceKernelDlsym (obs_module_symbol) into the same function pointers the payload
 * path uses, so the sink calls a data pointer it can null-check rather than a raw
 * import whose slot a title may leave at the unresolved sentinel. A no-op for a payload
 * (already bootstrapped) and where module resolution is unavailable. Call once at
 * entry, before the first write. See src/runtime.c. */
 ...
 * callable but not readable, so a check that dumps a resolved function's code bytes
 * faults. This queries the mapping (kernel virtual query) and refuses anything without
 * the read bit, and the eboot's own execute-only text outright. Zero on the host, where
 * there is nothing to query. See src/runtime.c and D325. */
