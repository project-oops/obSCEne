 ...
 * presentation concern and live in `obscene-tool pretty`, so the guest binary stays small
 * and the format stays single.
 *
...
/* The measured execution context of the run - "<delivery>/<generation>" plus a basis - emitted
 * alongside `build`. See obs_report_context in src/report.c and obs_run_context in runtime. */
 ...
 *
 * An observation, not a verdict - the raw input and output bit patterns of one lane, with
 * nothing asserted about them. The point is to compare these against an emulator's shader
 * translation, which is a diff the recipient does, not a pass this program can judge. Bits
 * rather than decimals because a reciprocal's last ULP is exactly where hardware and
 * emulation part company, and a rounded decimal would hide it. */
void obs_report_gpu(const char *kernel, unsigned int lane, uint32_t input, uint32_t output);

/* One lane of a multi-operand GPU kernel: the output, then each input.
 *
 * A separate record from `gpu` on purpose. That record's input field is a single value, and
 * a consumer already parses it; widening that field to a list would change a field's meaning,
 * which the contract forbids (docs/OUTPUT.md). This record puts the single output first and
 * the variable-length inputs after it, so a reader takes three fixed fields - kernel, lane,
 * output - and then as many inputs as the line still holds. That is how fma, pow, min and
 * max record their operands without disturbing the unary format. */
 ...
 *
 * Emitted when a serving build starts listening, so a reader of the report - on stdout, in
 * the file sink, or on the drawn screen - knows the module is waiting for the driver and
 * where to reach it. The port is the same fact the driver needs; on an emulator that maps
 * guest sockets to the host (shadPS4), the host connects to it on loopback. */
void obs_report_net(const char *state, unsigned int port);

/* One field of the on-screen status readout, mirrored into the report.
 *
 * The HUD's facts - memory, VRAM, generation, address - are what the platform says about
 * itself, and until now they lived only on the drawn screen. A driver reading the report
 * could see the socket come up but not a byte of the rest. This record carries each field
 * with its honest tier: `known` (read through a confirmed signature), `unconfirmed` (the
 * query resolves but its signature is not, so the value is `unknown`), or `absent` (no such
 * query here). The `state` field is the point - it is what keeps "unknown because missing"
 * distinct from "unknown because not yet wired".
 *
 * An observation, never a verdict, and never corpus provenance: a probe cannot certify its
 * own machine (D108), so these are recorded and diffed across platforms, not graded. See
 * src/sysinfo.c. */
 ...
 *
 * Emitted once, immediately after the build record, and **always** - a null path produces
 * a record saying so rather than no record at all. That asymmetry is the point: a missing
 * record is indistinguishable from an older build that never had a sink, whereas
 * `sink|none` is a fact about this run.
 *
 * See src/sink.c. */
void obs_report_sink(const char *path);

/* How many checks this run is skipping because a previous run of the same build announced
 * them and never finished, and whether the skip set filled up. */
 ...
 * question "did this behave correctly?" is uninteresting, because the answer is yes. What
 * cannot be got any other way is *what the platform actually wrote* - and a hexdump of a
 * buffer settles a struct layout permanently, where four experiments guessing at it
 * settle nothing.
 *
 * # And it routes around D008 rather than breaking it
 *
 * D008 forbids calling a function whose struct layout is unknown, because a wrong layout
 * produces a call that succeeds and does the wrong thing. Dumping bytes needs no layout:
 * it needs the arity, and a buffer larger than anything the call could write. The
 * expectation is what was missing, and this record does not have one.
 *
...
void obs_report_bytes(const char *id, const char *symbol, const char *what,
                      unsigned int offset, const unsigned char *bytes, unsigned int len);
...
/* Dumps a buffer against what it held before the call, so a field written as zero is not
 * mistaken for a field that was never written.
 *
 * `obs_report_buffer` above takes the last non-zero byte as the extent, which is right for a
 * buffer that started zeroed only while the platform never writes a zero. It does: a reserved
 * word, a count of none, a null pointer. Those are fields, and on hardware "wrote 0 here" and
 * "did not touch this" are different facts about a structure - one of them says the field
 * exists.
 *
 ...
 *
 * Call it twice with two different poison values and union the results if the answer matters:
 * a byte the platform happens to write equal to the poison reads as untouched, and no single
 * pattern avoids that. `obs_layout_patterns` is the pair this project uses. */
 ...
 *
 * The question this answers is "how big is that structure", and it answers it without anyone
 * deciding anything. A call given a size smaller than the structure it fills refuses; given
 * enough, it works. Walk a ladder of sizes and the boundary between the two **is** the size.
 *
 * That matters more than one more hexdump because it is `derived` rather than `assumed`: the
 * platform is the one drawing the line, and every call taking a size can be asked. (D008) */
 ...
 *
 * Emitted twice per symbol. The first carries outcome "attempt" and no value, before the
 * call; the second carries the classification and the returned value, after it. **A record
 * with no partner is the finding** - it names the function that ended the process, which
 * is the one thing a crash otherwise destroys the evidence for.
 *
 * `index` is the position in the census order, and it is what the resume harness reads. It
 * is in the record rather than derived from line count because a run that starts partway
 * through has no line count to derive it from.
 *
 * `outcome` labels the value by an explicit rule stated in src/sections/bulk.c; `returned`
 * is the raw answer, so a reader who disagrees with the label still has the fact. */
 ...
 * one. The established route is to guess - generate candidate names, hash them, and hope
 * for a match against identifiers seen in the wild. It works and it is enormously
 * expensive, and it can only ever find names that something happens to import.
 *
 * If the platform can look a symbol up **by name**, that whole problem collapses into one
 * question per candidate, with a yes or no answer - and it reaches functions no title
 * imports at all, which guessing never can.
 *
 ...
 * away the code that came back. On an emulator that is the right shape - the code is
 * invented anyway. On hardware the code **is the finding**: an implementation returning a
 * value no caller recognises makes guests retry forever, and a table of real codes turns
 * "unimplemented" into "fails the way the caller expects".
 *
 * `argument` describes what was wrong in words, because "null path" is what a reader needs
 * and the value alone does not say. */
 ...
 * lives, and that is a layout - the thing D008 forbids assuming. So `first` and `second`
 * are the two sixty-four-bit values at the start of the structure, named for their
 * *position* and not for their meaning. A reader who later learns the layout can
 * reinterpret them; one who trusts a field called `end` cannot.
 *
 * The authoritative record is the accompanying `bytes` dump. This exists because a memory
 * map is read as a list, and a list of hexdumps is not a list.
 *
