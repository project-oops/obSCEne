 ...
 * `sockaddr` or an `mmap` argument, and a wrong layout produces a call that succeeds
 * and does the wrong thing (D008).
 *
 ...
 *
 * The census carried `sceGnm*`, which is the *previous* generation's driver, and
 * nothing for the current one - the interface the emulator this exists to serve
 * actually targets. The blocker was provenance: the obvious name lists come from
 * dumping decrypted libraries, and inventing plausible names would fill the census with
 * confident-looking absences that mean nothing.
 *
 * # What closed it
 *
 * A current-generation emulator carries these as structured export attributes - NID,
 * name and library together - and an independent NID database carries most of the same
 * names. Both are public and neither required anything decrypted.
 *
 * # Every one is checked against our own hash
 *
 * A name is censused only where hashing it reproduces the identifier the source
 * records. That is not a formality: it rejected five, two of which were placeholders
 * with the identifier embedded in the name - a project marking a function it could not
 * name, which would have entered the census as a symbol that cannot exist.
 *
 ...
 * cannot be both censused (const char, uncallable) and called. Listen and Accept are
 * new, declared there too. What remains here is the part still probed for presence
 * only. */
