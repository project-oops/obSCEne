 ...
 * chosen; this is written to *as well*, because "can this system talk to a terminal"
 * and "can this system leave something behind" are different questions and a console
 * usually answers them differently.
 *
 ...
 *
 * Returns the path that worked, or NULL if none did or the platform has no file
 * support. The caller reports it, so that a run with a working sink and a run with none
 * are distinguishable afterwards - which they are not if this fails quietly. */
const char *obs_sink_open(void);

/* Writes bytes to the sink if it is open, and does nothing if it is not.
 *
 * Deliberately silent about failure. This sits underneath the reporting layer, so it
 * has no way to report anything without recursion, and a sink that has stopped closes
 * itself rather than interrupting a run that is still producing useful records
 * elsewhere. */
 ...
 *
 * That is a deliberate structure and not a style preference. What differs between
 * targets here is small and total: the same path list, the same write loop, the same
 * never-buffer rule, reaching a different set of five functions. A `#if` in the middle
 * of `sink.c` would put two targets' code in one file where only one of them is ever
 * compiled, so the untaken branch is never even parsed and rots quietly.
 *
 * A list in the Makefile says which backend a target gets, in one place, visibly.
 * Adding a target means adding a file and a line - not threading another condition
 * through code that already works.
 *
 * A descriptor is an `int` because every backend so far has one. A backend whose handle
 * is not an int would need this widened, and that is a change worth making deliberately
 ...
 *
 * The read side exists for one caller: the previous run's report.
 * `obs_sink_backend_open` truncates, so by the time the sink is writing there is
 * nothing left to learn from - which is why this is separate rather than a mode flag,
 * and why `start.c` calls the reader first. See `obs_resume_load`. */
 ...
 *
 * Written onto the `resume` record so the next run inherits them: one dangling `try` is
 * an accident as often as a hang, and skipping on the first sighting cost sixteen
 * thousand measurements behind a `complete` verdict. See `obs_resume_load`. (D181) */
