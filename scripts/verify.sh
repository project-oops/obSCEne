...
#
# The index is generated by `tools/split-decisions.sh --index obscene`, shared with the
# sibling repositories; this checks the two still describe the same set. It stopped
...
if (cd tool && cargo run --quiet -- decisions --root ..) >"$log" 2>&1; then
    ...
    cat "$log"
    note_failure "obscene-tool decisions"
fi
