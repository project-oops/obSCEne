 ...
 * on its own - a name in this list that fails a behavioural check is a different problem
 * from a name that is missing entirely. Run on a real console it is the authoritative
 * list, and the difference between the two is the gap an emulator has left.
 *
 ...
 * at the front are read: the size the platform wrote back, and the name that follows it.
 * Anything deeper would be a guessed offset producing confident nonsense, which is
...
static obs_result check_module_list(void) {
    ...
    if (rc != 0) {
        return obs_fail_code("the module list could not be read", (uint64_t)(uint32_t)rc);
    }
    if (written == 0) {
    ...
    if (written > OBS_MODULE_MAX) {
        return obs_partial_value("more modules than this can hold; the list is truncated",
                                 (uint64_t)written);
    }
...
static obs_result check_module_names(void) {
    ...
    for (unsigned int i = 0; i < obs_module_count; i++) {
        /* Handle 0x0 refuses regardless of layout - a hardware run showed the list starts with
         * it - so naming it would count a handle failure as a layout failure. Skip it; the
         * describable handles come after. */
    ...
    if (unreadable > 0) {
        return obs_partial_value("some modules could not be named", (uint64_t)unreadable);
    }
 ...
 * them - shadPS4 reports 35,337 of 35,337 present for libraries it does not implement, and
 * `900-surface/control` fails on exactly that basis (D156). A resolved address is the
 * loader's opinion.
 *
 * `sceKernelLoadStartModule` is a different question with a different kind of answer. It
 * returns the platform's own error code for a module it does not have, and a handle for one
 * it does. There is no stub to hide behind: either the module was loaded or a number says
 * why not.
 *
 * # Recorded, never asserted
 *
 * Nothing here states an expectation. This program does not know which modules a given
 * firmware ships, and inventing a list of "should be present" would put a guess where the
 * report should carry a fact - the same reasoning `130-layout` and `140-oracle` are built
 * on. Each attempt emits a `measure` record carrying the code the platform returned, and
 * the verdict is only about whether the *mechanism* worked at all.
 *
 * # Why so few modules, and why these
 *
 * Five, all named in the loader diagnostics this project has already collected from its own
 * runs - `libc.prx` and `libSceFios2.prx` are the two shadPS4 names when it refuses the
 * corpus build (D149). Naming a module is free; the cost is the call, and a call that
 * *starts* a module runs its initialiser. Five known names is enough to establish whether
 * the instrument answers at all, and a wider sweep is a decision to make once it has.
 */
...
static obs_result check_module_load(void) {
    ...
    for (unsigned int i = 0; i < OBS_COUNT(obs_module_paths); i++) {
        int started = 0;
        /* The trailing arguments are the documented no-argument form: no argv, no flags,
         * no options, and a place to put the module's own return value. */
    ...
    /* Zero loaded is a real answer, not a failure: a firmwareless loader has no modules to
     * give, and it said so with a code per attempt. What would be a failure is the call
     * never returning, which the announcement above would name. */
    return obs_pass_value((uint64_t)loaded);
}

/* Does a loaded module yield a symbol by name?
 *
 * The other half of the runtime question. A handle alone says a module exists; resolving a
 * name through it says the module carries that symbol - which is what the address census
 * claims and cannot substantiate. Skipped rather than guessed when nothing loaded, because
 * asking a handle nobody obtained is not a question. */
 ...
 *
 * The last hardware run set this field to the buffer size - 1024 - and every module came
 * back refused with the invalid-argument code, while `sceKernelGetModuleList` in the same
 * run happily returned thirty-one handles. So the platform will describe modules; it would
 * not accept the *call*.
 *
 * A kernel filling a caller-supplied structure has to know which layout the caller compiled
 * against, and the usual way it finds out is this field. "Big enough" is then exactly wrong:
 * the value has to match a size the kernel recognises, and 1024 is not one. Sweeping finds
 * the size it wants without anybody having to know the layout first - which is the whole
 * argument for measuring a structure rather than guessing it (D008).
 *
 * The rungs are ordinary structure sizes for something holding a name, a few paths and a
 * handful of segment descriptors, plus round numbers either side. */
...
/* Written past the declared size and checked afterwards, so a call that writes more than it
 * was told is caught here rather than corrupting whatever followed. */
 ...
 *
 * A hardware run showed the list begins with handle `0x0`, and `sceKernelGetModuleInfo(0, ...)`
 * refuses with the invalid-argument code no matter what size it is handed - so the size ladder
 * built against handle[0] rejected everything and said nothing about the layout, when the
 * layout was never what it was testing (D008: a check that fails for the wrong reason reports a
 * wrong finding). The console enumerates real modules at `0x2`, `0x11`, `0x13` and up; this
 * picks the first of those.
 *
 * `0x2001` is skipped as well: it is a live handle in the list, but it is the one the payloads
 * treat as a system module and its describe behaviour may differ from an ordinary one - so the
 * layout is measured against a plain module first, and the special case is its own question. */
...
static obs_result check_module_info_size(void) {
    if (obs_module_count == 0) {
        return obs_skip("the module list was not read, so there is nothing to describe");
    }

    int handle = obs_first_describable_handle();
    if (handle == 0) {
        return obs_skip("no describable handle in the list - only 0x0 and the system handle");
    }
    ...
    for (unsigned int i = 0; i < OBS_COUNT(obs_module_info_sizes); i++) {
        ...
        /* The full sixty-four bits, little-endian. The neighbouring check writes only two
         * bytes, which is the same number for the values it used - but a ladder that climbs
         * past sixteen bits would silently stop changing the field. */
        for (unsigned int b = 0; b < 8; b++) {
        ...
        /* Accepted. Check the guard before reading anything: a call that overran the size it
         * was given has already written somewhere it should not, and the dump below would be
         * describing damage rather than a layout. */
        int overran = 0;
        ...
        if (overran) {
            return obs_fail_code("the call wrote past the size it was given", (uint64_t)size);
        }

        accepted++;
        if (smallest_accepted == 0) {
            smallest_accepted = size;
            /* The first size that works is the one worth dumping, and only that one: the
             * point is a layout, and repeating the dump for every larger size that also
             * works would bury it. */
            obs_report_buffer("110-modules/info-size", "sceKernelGetModuleInfo", "info", info,
                              size);
        }
    }
    ...
    obs_report_measure("110-modules/info-size", "sceKernelGetModuleInfo", "smallest-accepted",
                       (uint64_t)smallest_accepted, "bytes");
 ...
 *
 * The naming check reports a handle only alongside a name, so a run where naming fails - as
 * the last hardware run did - reports no handles at all. That threw away the more useful
 * half: the last run's `sceKernelLoadStartModule` records showed application modules getting
 * small integers and a system module getting `0x2001`, and the sibling emulator has two
 * payloads dying on exactly that constant, never having identified it. It is a handle.
 *
...
static const obs_check module_checks[] = {
    {"110-modules/load", "libkernel", "sceKernelLoadStartModule", OBS_CAP_NONE,
     ...
     OBS_FROM_ASSUMED},
    {"110-modules/symbol", "libkernel", "sceKernelDlsym", OBS_CAP_NONE,
     OBS_CAP_NONE, (const void *)&sceKernelDlsym, check_module_symbol,
     OBS_FROM_ASSUMED},

    {"110-modules/list", "libkernel", "sceKernelGetModuleList", OBS_CAP_NONE,
     OBS_CAP_NONE, (const void *)&sceKernelGetModuleList, check_module_list, OBS_FROM_ASSUMED},
    {"110-modules/names", "libkernel", "sceKernelGetModuleInfo", OBS_CAP_NONE,
     OBS_CAP_NONE, (const void *)&sceKernelGetModuleInfo, check_module_names, OBS_FROM_ASSUMED},
...
const obs_section obs_section_modules = {
    ...
    "authoritative; against an emulator it is what that emulator claims to provide, and "
