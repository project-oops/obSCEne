 ...
 * buffer and write PM4 command packets into it; they touch no GPU and submit nothing,
 * so calling them is as safe as any buffer-filling call (the same class as 130-layout),
 * and what they write is the PM4 encoding - the bytes a command-processor emulator has
 * to parse. That encoding is the finding: capture it here, and on hardware day diff it
 * against what an emulator produces.
 *
 * # Why only these two (D008)
 *
 * The vendor documents none of this. Their arities are confirmed instead by two
 * independent open reimplementations that agree exactly - shadPS4 and GPCS4 - which is
 * the same standard that let the sceNet transport be built (D107). `sceGnmSetCsShader`
 * is left out precisely because those two sources *disagree* on it (three arguments or
 * four), and D008 forbids calling a function whose arity is uncertain: a wrong one
 * corrupts the stack and surfaces far from here. The submitting calls
 * (`sceGnmSubmitCommandBuffers`) and the shader-binding ones stay in the census,
 * uncalled - reaching a real compute result through Gnm needs a GCN shader this build
 * does not produce, which is a separate workstream.
 *
 * # Where it runs
 *
 * Nowhere, on the host build: libSceGnmDriver is absent, so the harness skips both
 * checks as "not present". It runs inside a loader that provides sceGnm - shadPS4 - the
 * same way the sceNet transport is exercised without a console. The PM4 it records
 * there is that emulator's encoding; on real hardware it is the vendor's, and the two
 ...
 * that writes further than any of these should - the one fault this section could cause
 * and not otherwise notice. D008-safe: dumping bytes needs the arity and a buffer
 * larger than the call can fill, not the struct layout. */
...
/* The last non-zero dword's index plus one: how many command dwords the call actually
 * wrote, which is the answer to "how big is this packet" that costs the most to
 * establish otherwise. */
...
/* Dumps the command buffer as bytes, unless the call overran - in which case that is
 * said first and loudly, because nothing after an overrun is trustworthy. Returns 1
 * when the buffer was dumped and the caller may judge it, 0 when it overran and the
 * caller should stop. */
static int gnm_dump(const char *id, const char *symbol, const gnm_probe *probe,
                    obs_result *bad) {
...
static obs_result check_gnm_dispatch_init(void) {
    ...
    gnm_prepare(&probe);
    uint32_t reserved =
        sceGnmDispatchInitDefaultHardwareState(probe.cmdbuf, OBS_GNM_DWORDS);
    obs_result bad;
    if (!gnm_dump("165-gnm/dispatch-init", "sceGnmDispatchInitDefaultHardwareState",
                  &probe, &bad)) {
        return bad;
    }
    if (reserved == 0u) {
...
/* sceGnmDispatchDirect(cmdbuf, size, x, y, z, flags): writes a compute dispatch of
 * x*y*z thread groups as PM4, and requires size to be exactly 9. Returns 0 on success,
 * negative on a rejected argument - a refusal is not a failure of this section, only
 * the call telling us its rule. */
...
static const obs_check gnm_checks[] = {
    {"165-gnm/dispatch-init", "libSceGnmDriver",
     ...
     (const void *)&sceGnmDispatchInitDefaultHardwareState, check_gnm_dispatch_init,
     OBS_FROM_ASSUMED},
...
const obs_section obs_section_gnm = {
    ...
    "Calling the confirmed libSceGnmDriver command-builders and recording the PM4 they "
    "encode.",
    gnm_checks,
