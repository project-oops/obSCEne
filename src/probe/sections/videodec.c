...
static obs_result check_videodec2_create_present(void) {
    ...
    obs_report_measure("107-videodec/create-present", "sceVideodec2CreateDecoder",
                       "vaddr", (uint64_t)(uintptr_t)addr, "offset");
    return obs_pass();
}

/* The decode call itself - the one that would turn an access unit into a frame in
 * memory the display can sample. A named row for it, resolution only. */
