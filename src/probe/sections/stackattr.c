 ...
 *
 * `scePthreadAttrGetstackaddr` hands back an address. **Is that address the lowest byte of
 * the stack, or its top?** The two conventions differ by the whole size of the stack, and
 * a caller that guesses wrong computes a bound that is megabytes away from the real one.
 *
 * It is not an academic question. Three retail titles run the same sequence once each,
 * early: `scePthreadSelf`, then `scePthreadAttrGet` on the answer, then the stack address
 * and the stack size. That is a garbage collector finding the span it has to scan. The
 * sibling emulator left those calls unimplemented, the collector computed its bound from a
 * placeholder, and all three titles walked off the top of the real stack into the first
 * unmapped page above it (its D575). It now answers them on FreeBSD's convention - address
 * is the lowest byte - and records that as an assumption a probe could settle. This is that
 * probe.
 *
 * # How it is settled without assuming either answer
 *
 * The check takes the address of one of its own locals, which is by construction inside the
 * stack of the thread asking, and asks which side of the reported address it falls:
 *
 *   * inside `[address, address + size)` - the address is the **base**, FreeBSD's convention;
 ...
 *
 * No layout is assumed, no constant is invented, and the arithmetic is the whole method.
 * `sceKernelIsStack` is consulted as a second, independent witness where it resolves - it
 * reports bounds of its own - but nothing is asserted on it, because platform.h is explicit
 * that those two out-pointers are only *almost certainly* the region's bounds.
 *
 * # Nothing here blocks and nothing is dereferenced
 *
 * Every value is compared as an integer. The one pointer that gets written through is an
 * out-parameter the caller owns. A bogus thread handle is deliberately not probed: the
 * platform would have to look it up in its own thread list, and handing a made-up pointer
 * to that is a crash risk that would cost every check behind this one to learn a code.
 */
...
/* What the section learned about the running thread, filled by the first check and read by
 * the second. Kept rather than re-derived so the two checks describe one observation: a
 * second `AttrGet` could in principle answer differently, and then the classification would
 * be of a region the first check never reported. */
...
static obs_result check_self_describes(void) {
     ...
     * out-parameter left untouched, which is the failure the whole family keeps meeting. */
    if (size == 0) {
        return obs_fail("a running thread was described as having a stack of no size");
    }
    if (address == NULL) {
        return obs_fail("a running thread was described as having a stack at no address");
    }
 ...
 *
 * The finding is the classification, and it is reported as a measurement whichever way it
 * comes out: `1` for a base, `2` for a top. Only "neither" is graded, because a region that
 * does not contain the frame asking about it is wrong under both conventions. */
...
static obs_result check_address_is_the_base(void) {
    ...
    if (!s_described) {
        return obs_skip("the calling thread was not described, so there is nothing to place");
    }
    ...
    obs_report_measure("031-stackattr/address-is-the-base", "scePthreadAttrGetstackaddr",
                       "placement", (uint64_t)placement, "placement");
    obs_report_measure("031-stackattr/address-is-the-base", "scePthreadAttrGetstackaddr",
                       "frame-address", (uint64_t)here, "address");

    /* The second witness, where it resolves. `sceKernelIsStack` reports bounds of its own,
     * and platform.h is explicit that they are only almost certainly the region's - so they
     * are recorded beside the classification and nothing is asserted on them. Guarded on
     * its own rather than through OBS_REQUIRE, so a platform without it still answers the
     * question this check exists for. */
    if (obs_address_is_callable((const void *)&sceKernelIsStack)) {
        ...
        obs_report_measure("031-stackattr/address-is-the-base", "sceKernelIsStack", "low",
                           (uint64_t)(uintptr_t)low, "address");
        obs_report_measure("031-stackattr/address-is-the-base", "sceKernelIsStack", "high",
                           (uint64_t)(uintptr_t)high, "address");
    }

    if (placement == OBS_SA_ADDRESS_IS_NEITHER) {
        /* Neither end. The frame asking is not inside the region either reading would give,
         * so the description belongs to some other stack or to none. */
        return obs_fail_code("the described stack does not contain the frame that asked",
                             (uint64_t)reported);
    }
 ...
 *
 * A getter on an initialised attribute object succeeds - POSIX settles that much, which is
 * why this is DERIVED - and what it *reports* for a stack nobody set is the measurement.
 * A caller that reads a stale or invented region out of a fresh set would compute a bound
 * from it exactly as confidently as from a real one. */
static obs_result check_fresh_attr_names_no_stack(void) {
    OBS_REQUIRE(&scePthreadAttrInit, &scePthreadAttrDestroy, &scePthreadAttrGetstackaddr,
    ...
    if (addr_rc != 0 || size_rc != 0) {
        return obs_fail_code("a getter refused an attribute set it had just initialised",
                             (uint64_t)(uint32_t)(addr_rc != 0 ? addr_rc : size_rc));
    }
    /* The out-parameters were seeded with a recognisable value, so a platform that accepts
     * the call and writes nothing is visible in the record above rather than reading as a
     * plausible address. */
...
static const obs_check stackattr_checks[] = {
    ...
    {"031-stackattr/fresh-attr-names-no-stack", "libkernel", "scePthreadAttrGetstackaddr",
     OBS_CAP_NONE, OBS_CAP_NONE, (const void *)&scePthreadAttrGetstackaddr,
     check_fresh_attr_names_no_stack, OBS_FROM_DERIVED},
};

const obs_section obs_section_stackattr = {
    "031-stackattr",
    "Thread stack attributes",
    "What a thread attribute set reports about a running thread's stack, and - the open "
    "question a collector's scan bound depends on - whether the address it gives is the "
