 ...
 * "does this work?" but "is this here?", which costs only a name. This asks the question
 * in between, and it is the one nothing else in the suite can answer:
 *
 ...
 * emulator that resolves every import to a shared do-nothing stub scores a perfect census
 * - 383 of 383 present - while implementing none of it. `docs/COMPATIBILITY.md` says this
 * outright and has had no instrument for it.
 *
 ...
 * |---|---|
 * | a vendor error code | argument validation ran, so something real is behind the symbol |
 * | zero | success on null input, or a stub that returns zero - the two are not separable |
 * | did not return | the callee dereferenced an argument without checking it, or blocked |
 *
 ...
 *
 * That section answers the same question far better, for a hundredth as many symbols. It
 * calls a function twice with inputs whose answers must differ and compares the two - so
 * it proves the function reads its arguments, which is real evidence. It costs a confident
 * signature and a hand-written probe pair per symbol, which is why it covers 54.
 *
 ...
 * callee ignores what it does not read. This is the same property that lets `printf` work.
 *
 * *Nothing can succeed and do the wrong thing silently,* because nothing here has an
 * expectation to be wrong about. `130-layout` and `140-oracle` route around D008 the same
 * way and for the same reason: a record is not a claim.
 *
 * What is genuinely given up is that **the return value is only meaningful for functions
 * returning an integer**. One returning a float leaves the answer in a vector register and
 * this reads the integer one, which holds whatever was there. One returning a large struct
 * by value is handed a null result pointer and will fault. Both are recorded as what they
 * are and neither is corrected, because correcting them needs the signature this section
 ...
 * holding whatever was there. Calling through a variadic prototype makes the compiler set
 * it to zero, which is correct for a variadic callee and ignored by every other kind - so
 * it is strictly safer than the obvious prototype, at no cost.
 *
 ...
 * function that faults. Recompiling with the index after it resumes immediately past the
 * crash instead of paying for the whole sweep again - the same shape as the exclusion list
 * in `sweep-build.sh`, and for the same reason. */
 ...
 *
 * The section itself always exists and always reports - one skip, with the reason - so a
 * default run says "this was not done" rather than saying nothing. A capability that
 ...
 *
 * This is the point in the program where that convention is deliberately stepped around,
 * and doing it by casting an address rather than by redeclaring the name keeps the step
 * local to one expression. Every other translation unit still gets a `const char` and
 * still cannot call these by accident - which is the whole value of the convention, and
 * would be lost by declaring them as functions here. */
 ...
 * expansion did exactly that and died at `libSceImageUtil|__dso_handle`. Counting a data
 * symbol is fine; calling it is not, so the generator emits two lists and this takes the
 * narrower one (D114). */
...
static const bulk_group groups[] = {
    OBS_SURFACE_LIBRARIES(OBS_BULK_GROUP_ROW)
    OBS_CORPUS_CALLABLE_LIBRARIES(OBS_BULK_GROUP_ROW)
};
...
static const char *classify(uint64_t returned) {
    ...
    if ((returned >> 63) != 0 || (returned & 0x80000000ull) != 0) {
        /* Some other negative-looking answer: a different facility, or an errno returned
         * directly. Worth separating from an ordinary value without claiming to know
         * which. */
        return "error-shaped";
    }
    return "value";
...
static obs_result run_bulk(void) {
     ...
     *
     * Every other record here carries a bare position, and a position with no denominator
     * beside it invites the reader to supply one - which they will do from whatever count
     * they last saw. Two different counts in this repository are both called "the census":
     * `surface.h` holds 371 curated symbols and gets printed by `verify.sh`, and this
     * section walks the callable corpus, which is roughly a hundred times larger and was
     * printed nowhere. A sweep that had covered 1.1% of the corpus was read, recorded and
     * reported as 95% of the census on exactly that basis. (D163)
     *
     * Emitted from the group table rather than from a constant, so it cannot disagree with
     * what the loop below actually walks. */
     ...
     *
     * It counts *every* symbol including the ones not resolved, rather than counting only
     * the ones called. Two runs of the same build must agree on what position 291 means, and
     * a count that skips absences does not - it would shift the moment a loader resolved
     * one more symbol, which is exactly the situation where a resumed sweep is being
     * compared against the run it resumed from. */
    for (unsigned int g = 0; g < OBS_COUNT(groups); g++) {
        for (unsigned int t = 0; t < groups[g].count; t++, position++) {
            ...
            /* Not `!= NULL`, for the reason the census gives: a loader resolving unknown
             * symbols to a small non-null value would have every one of them called. */
             ...
             * survivable. A record with an position and no answer after it names the exact
             * function that ended the process - which is both the finding and the input
             * the harness needs to resume. Principle 1, at the one place in the program
             * where it is load-bearing rather than precautionary. */
