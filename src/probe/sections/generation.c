...
/* Two more markers, taken over from `sysinfo.c` when its duplicate inference was
 ...
 *
 * Extracted so there is exactly one inference. `sysinfo.c` had its own, drawing the
 * header that reads `GEN 5 (CURRENT)` - and it used *different* marker symbols
 * (`sceGnmSubmitDone`, `sceAgcAcbAcquireMem`) than this section did. Two inferences
 * that can disagree, over the one fact the report puts in its header, while the
 * accessor built to prevent that had no callers at all (D110).
 *
 * More markers rather than fewer: a platform that implements one function of a driver
 ...
 *
 * This read `&sceAgcCreateShader != NULL`, which is presence-by-linking: taking the
 * address imports the name, which puts `libSceAgcDriver` in `DT_NEEDED`, which makes a
 * system loader load it before this program runs. The two AGC libraries are
 * current-generation graphics and a package installs under a `ps4_game` category, so
 * they are exactly the ones a title is least likely to be given - and a `DT_NEEDED` a
 * title cannot meet is a console that dies with nothing on record, before any of this
 * could report. (D226)
 *
 * That made the generation probe able to kill the thing it was probing. Asking instead
 * returns a value: the driver loads, or it does not, and either way the answer is
 * legible.
 *
...
static int obs_current_present(void) {
     ...
     * this the probe reports "the current generation's driver is missing" on a platform
     * where it simply did not look. Measured on PS5PCEM, which implements the current
     * generation's graphics and was reported as neither generation. (D232) */
     ...
     * `libSceAgc` and `libSceAgcDriver`, and those are `EI_ABIVERSION 2` libraries. A
     * title in the previous generation's category is refused them by the loader, which
     * says so in the system log and not to us:
     *
     *     ### ERROR: ABIVERSION mismatch. /<sandbox>/common/lib/libSceAgcDriver.sprx
     *
     * So on a current-generation console this returned 0, `005-generation/detect`
     * passed, and the report and the on-screen header both said **GEN 4** about a
     * machine that is not one. A pass with a wrong answer is worse than a failure,
     * because nothing about it invites a second look.
     *
     * The distinction is the one this function already makes at the top for
     * `obs_module_resolution_works`, applied one level down: a library that did not
     * open tells you nothing about what is in it. A library that opened and did not
     * have the symbol tells you the symbol is not there, and that is still worth
     * reporting as absence. (D255) */
    int opened_any = 0;
 ...
 * `005-generation` would be unknown for the first part of every run - and the honest
 * fix is to compute it when asked rather than to keep a second copy of the reasoning
 * somewhere it can be answered earlier. */
obs_generation obs_detected_generation(void) {
     ...
     *
     * This computed `previous && !current` and called it *previous generation*, which
     * reads the same for "the current generation's markers are absent" and "the current
     * generation's markers could not be reached". On a real console it is always the
     * second: the markers are `EI_ABIVERSION 2` libraries and this module's category is
     * refused them.
     *
     * The result was a console reporting **GEN 4** about itself, in the stream and
     * across the top of its own screen, with nothing anywhere saying the answer was a
     * guess. Unknown is the honest output and it is visibly unknown, which is the whole
     * point of having the state. (D255) */
    if (answer < 0) {
        return detected;
    }
...
/* The graphics driver(s) present, reported as a driver fact rather than a generation
 * verdict.
 *
 * `obs_detected_generation` collapses to UNKNOWN the moment the current generation's
 * driver cannot be looked at, which on a ps4_game is always (D255) - and that discards
 * the gnm driver that is plainly here (it draws the report and `165-gnm` passes). This
 * asks the same two markers a different question - "which driver resolves", not "which
 * console is this" - so the gnm the verdict has to drop is still surfaced. `== 1` on
 * the current side, so a "could not look" (-1) is not read as present; gnm being
 * present names no console, so nothing here ages. */
...
static obs_result check_generation(void) {
    ...
    /* The current-generation side could not be looked at, so no verdict is available
     * even though half the evidence is. Reported as its own outcome rather than folded
     * into "neither driver resolves", which would be a claim about the platform made
     * from a measurement that never happened. (D232) */
    if (current_raw < 0) {
        /* Unknown, even though the previous generation's driver resolves.
         *
         * This wrote `previous ? PREVIOUS : UNKNOWN`, which is the exact "GEN 4 about
         * itself" bug D255 fixed in `obs_detected_generation` - and missed here, in the
         * check that actually runs. The two share the cached `detected`, so the correct
         * UNKNOWN from the header's early call was overwritten with PREVIOUS the moment
         * this section ran, and every later redraw showed GEN 4 while the stream
         * (captured before this) still said unknown. The console cannot be identified
         * when the current generation's markers cannot even be loaded, so the cache
         * must say so; the message below still notes that the previous driver resolved,
         * which is the informative part. (D255) */
        detected = OBS_GENERATION_UNKNOWN;
        return obs_partial(
            previous
                ? "the previous generation's driver resolves; the current generation's "
    ...
    if (previous && current) {
        /* Both drivers resolve. On real hardware that can be the back-compat path; on
         * an emulator it is more often the fingerprint of a loader that stub-resolves
         * every unresolved import, so the current-generation driver "answers" with no
         ...
         * generation, so this stays a non-verdict rather than picking one - the same
         * reason the header renders `GEN` as `unknown` here rather than a console
         * number. */
        detected = OBS_GENERATION_BOTH;
        return obs_partial(
            "both generations' drivers resolve (real back-compat, or a "
