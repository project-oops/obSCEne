...
static const char *const obs_video_recording_symbols[] = {
    ...
    "sceVideoRecordingQueryMemorySize",
    "sceVideoRecordingSetStatus",
...
static void *obs_find_symbol_in_handle(int handle, const char *name) {
    ...
    if (obs_address_is_callable((const void *)&sceKernelDlsym)) {
        void *addr = NULL;
        if (handle > 0 && sceKernelDlsym(handle, nid, &addr) == 0 && obs_address_is_callable(addr)) {
            return addr;
        }
        if (handle > 0 && sceKernelDlsym(handle, name, &addr) == 0 && obs_address_is_callable(addr)) {
            return addr;
        }
    }
    return NULL;
...
static obs_result check_encoder_sysmodules(void) {
    ...
    for (size_t i = 0; i < OBS_COUNT(venc_modules); i++) {
        ...
        obs_report_measure("106-encoder/sysmodule-load", venc_modules[i].name, "id", (uint64_t)venc_modules[i].id, "id");
        obs_report_measure("106-encoder/sysmodule-load", venc_modules[i].name, "rc", (uint64_t)(uint32_t)rc, "code");
...
static obs_result check_encoder_module_load(void) {
    ...
    for (size_t i = 0; i < OBS_COUNT(search_paths); i++) {
        if (obs_address_is_callable((const void *)&sceKernelLoadStartModule)) {
            ...
            obs_report_measure("106-encoder/path-probe", search_paths[i], "handle", (uint64_t)(uint32_t)h, "handle");
            obs_report_measure("106-encoder/path-probe", search_paths[i], "res", (uint64_t)(uint32_t)res, "code");
    ...
    obs_report_measure("106-encoder/module-list", "total", "count", (uint64_t)count, "modules");

    for (size_t i = 0; i < count && i < 128; i++) {
        if (handles[i] <= 0) continue;
        obs_report_measure("106-encoder/module-handle", "loaded", "handle", (uint64_t)(uint32_t)handles[i], "handle");
    }
    ...
    if (count > 0) {
        return obs_partial_value("paths returned error codes; module list read", (uint64_t)count);
    }
...
static obs_result check_encoder_symbol_census(void) {
    ...
    for (size_t i = 0; i < OBS_COUNT(obs_venc_symbols); i++) {
        ...
        if (addr != NULL) {
            resolved++;
            obs_report_measure("106-encoder/symbols", name, "vaddr", (uint64_t)(uintptr_t)addr, "offset");
            obs_report_measure("106-encoder/symbols", name, "handle", (uint64_t)(uint32_t)found_handle, "handle");
            if (obs_address_is_callable(addr)) {
                obs_report_buffer("106-encoder/prologue", name, "code", (const unsigned char *)addr, 16);
            }
        } else {
    ...
    for (size_t i = 0; i < OBS_COUNT(obs_video_recording_symbols); i++) {
        ...
        if (addr != NULL) {
            obs_report_measure("106-encoder/rec-symbols", name, "vaddr", (uint64_t)(uintptr_t)addr, "offset");
            obs_report_measure("106-encoder/rec-symbols", name, "handle", (uint64_t)(uint32_t)found_handle, "handle");
            if (obs_address_is_callable(addr)) {
                obs_report_buffer("106-encoder/rec-prologue", name, "code", (const unsigned char *)addr, 16);
            }
        } else {
            obs_report_measure("106-encoder/rec-symbols", name, "unresolved", 0, "status");
        }
    }
...
static obs_result check_encoder_create_present(void) {
    ...
    if (addr != NULL) {
        obs_report_measure("106-encoder/create-present", "sceVencCoreCreateEncoder", "vaddr", (uint64_t)(uintptr_t)addr, "offset");
        return obs_pass();
    }
...
static obs_result check_encoder_getaudata_present(void) {
    ...
    if (addr != NULL) {
        obs_report_measure("106-encoder/getaudata-present", "sceVencCoreGetAuData", "vaddr", (uint64_t)(uintptr_t)addr, "offset");
        return obs_pass();
    }
...
static obs_result check_encoder_query_present(void) {
    ...
    if (addr != NULL) {
        obs_report_measure("106-encoder/query-present", "sceVencCoreQueryMemorySize", "vaddr", (uint64_t)(uintptr_t)addr, "offset");
        return obs_pass();
    }
...
static obs_result check_related_video_modules(void) {
    ...
    for (size_t i = 0; i < OBS_COUNT(related_libs); i++) {
        ...
        if (h > 0) {
            found++;
            obs_report_measure("106-encoder/related-libs", related_libs[i], "handle", (uint64_t)h, "handle");
        } else {
            obs_report_measure("106-encoder/related-libs", related_libs[i], "absent", 0, "status");
        }
    }
...
const obs_section obs_section_encoder = {
    ...
    "Whether libSceVencCore and related video encoding libraries load at runtime, and whether "
