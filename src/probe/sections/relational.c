 ...
 *
 * There is a way to test without an oracle: compare results **to each other** rather
 * than to an expected value. Two calls that must differ, an operation and its inverse,
 * a sequence that must be repeatable. These hold whatever the platform's actual numbers
...
static obs_result check_event_flag_handles_distinct(void) {
    ...
    for (unsigned int i = 0; i < COUNT; i++) {
        int rc =
            sceKernelCreateEventFlag(&flags[i], "obscene-distinct",
...
static obs_result check_event_flag_handles_reusable(void) {
    ...
    for (unsigned int round = 0; round < ROUNDS; round++) {
        ...
        int rc = sceKernelCreateEventFlag(
            &flag, "obscene-reuse", OBS_EVF_ATTR_FIFO | OBS_EVF_ATTR_SINGLE, 0, NULL);
        if (rc != 0) {
            if (round == 0) {
            ...
            return obs_fail_code(
                "creation stopped working after repeated create/delete",
                (uint64_t)round);
        }
...
static obs_result check_direct_memory_round_trip(void) {
    ...
    sce_off_t first = 0;
    int rc =
        sceKernelAllocateDirectMemory(search, (sce_off_t)sceKernelGetDirectMemorySize(),
...
static obs_result check_descriptors_distinct(void) {
     ...
     * the other's. What breaks is somewhere else, later, in code that did nothing
     * wrong.
     *
     ...
     * least invasive way to ask "is this still a live descriptor" - it moves nothing
     * and needs no buffer. */
    (void)sceKernelClose(first);
...
static obs_result check_clock_never_goes_backwards(void) {
     ...
     * clock is not failed for being coarse: equal readings are fine, decreasing ones
     * are not.
     *
...
static obs_result check_mutex_is_not_recursive_by_default(void) {
     ...
     * deadlocking the run - which is the failure this relation would otherwise cause
     * and is exactly what CLAUDE.md forbids.
     *
     * Marked assumed rather than spec: POSIX makes the default type implementation-
     * defined, so "not recursive" is a strong expectation and still this project's.
     * What the check really establishes is that the lock *tracks state at all* - an
 ...
 *
 * The three checks below are one idea applied to three subsystems, and it is the idea
 * the rest of this section does not cover.
 *
 ...
 * single global word passes all of them - the count counts, the bits set, the handles
 * are distinct because they are allocated separately from the state they name. It is
 * only visible by asking a **second** object whether it can see the first one's state.
 *
 * That is not a hypothetical shape. It is what the simplest possible stand-in looks
 * like: one static variable and a function that reads it, which is how a subsystem gets
 * stubbed when the API is needed before the implementation is.
 */

static obs_result check_event_flag_state_is_per_object(void) {
    OBS_REQUIRE(&sceKernelDeleteEventFlag, &sceKernelPollEventFlag,
     ...
     * of these calls return on failure, and a bit that reads the same as "nothing"
     * makes the two outcomes indistinguishable in the record. */
     ...
     * consume or clear the bit on an implementation that does more than it is asked,
     * and the interesting question would then be asked of a flag whose state we had
     * already disturbed. */
    ...
    if (set != 0) {
        return obs_fail_code("a fresh event flag refused a set",
                             (uint64_t)(uint32_t)set);
    }
    if (seen_own != 0) {
 ...
 *
 * `semaphore-state-is-per-object` creates two semaphores into two adjacent `int`s and
 * then signals the first. On fpPS4 that failed with "a fresh semaphore refused a
 * signal" and the code `0x80020016` - EINVAL - which sent a reader looking at the
 * *signal*. The signal was fine. `sem_enter` returns EINVAL for a null handle, and the
 * handle was null because
 * **creating the second semaphore had overwritten the first**.
 *
 * `sceKernelCreateSema` takes an `int *`. fpPS4's `SceKernelSema` is a pointer to a
 * struct, so it writes eight bytes through it; shadPS4's is a `u32` slot index and
 * PS5PCEM's parameter is
 * `?*u32`, both four. Two implementations against one, and the declaration here follows
 * the two - which makes the third an overrun rather than a disagreement about arity.
 *
 * # Why it needs its own check
 *
 * The relation check found it by accident and described it wrongly, because a corrupted
 * handle fails at whatever touches it next. Asking the question directly turns
 * "something later went wrong" into "the call wrote past its argument", and that is the
 * difference between a bug report an implementer can act on and one they cannot
 * reproduce.
 *
 * # The shape
 *
 * A guard word immediately after the handle, checked for disturbance. This is
 * `130-layout`'s trick applied to a four-byte destination: the buffer there is
 * oversized and the guard proves nothing ran past it. Same idea, smaller subject.
 *
 * Reported rather than judged where it is ambiguous. A platform whose handle is
 * genuinely pointer-sized is not committing an error by writing eight bytes - it is
 * disagreeing with this program's declaration, and the record says which happened
 * rather than assuming. What is never acceptable is the silent half: a caller who
 * allocated four bytes lost whatever sat in the next four, and nothing told them.
 * (D171)
 */
static obs_result check_handle_fits_its_out_parameter(void) {
    OBS_REQUIRE(&sceKernelCreateSema, &sceKernelDeleteSema);

    /* Guard immediately after the handle, in one struct so the adjacency is the
     * compiler's contract rather than a hope about stack layout - which is exactly what
     * made the original failure depend on which order two locals happened to be placed
     * in. */
    struct {
        int handle;
        uint32_t guard;
    ...
    obs_report_measure("018-relational/handle-fits-its-out-parameter",
                       "sceKernelCreateSema", "guard-after-handle", (uint64_t)after,
                       "word");
    if (after != 0xA5A5A5A5u) {
        /* The four bytes past the handle were written. The value is what landed there,
         * which for a pointer-sized handle is its upper half and says so at a glance.
         */
...
static obs_result check_semaphore_state_is_per_object(void) {
     ...
     *
     * Two bare adjacent `int`s here is what found the fpPS4 out-parameter overrun
     * (D171), and it found it *badly*: creating the second handle overwrote the first,
     * the first then signalled as null, and this check reported "a fresh semaphore
     * refused a signal" - which is about the signal, which was fine. A corrupted handle
     * fails at whatever touches it next, and that is never where the fault is.
     *
     * So the relation is only asked once the handles are known intact. Where they are
     * not, this **skips and names the check that measures it**, because on such a
     * platform the question cannot be asked: a handle wider than its slot is unusable
     * through that slot, and failing the relation would blame per-object state for a
     * width problem. */
    struct {
        int handle;
        uint32_t guard;
    ...
    if (a.guard != 0xA5A5A5A5u || b.guard != 0xA5A5A5A5u) {
        ...
        return obs_skip(
            "the handle does not fit its out-parameter on this platform, so it "
    ...
    /* The untouched one first, for the same reason as above: a successful poll consumes
     * a count, so asking the signalled one first would change what the second question
     * is about. */
    ...
    if (took_from_own != 0) {
        return obs_fail_code(
            "a semaphore would not yield the count just signalled to it",
 ...
 *
 * `scePthreadJoin` blocks, and this section runs at 018 while `030-thread` is where
 * join is established. A join here on a platform whose threads do not finish would
 * hang, and take every check behind it - including the ones in `030-thread` that would
 * have diagnosed it. The section's own rule is that anything which can block is written
 * as the `try` form or not at all, and there is no `tryjoin`.
 *
 * So the child sets a flag last and the parent spins a bounded number of times waiting
 * for it. The parent gives up rather than waiting, and the give-up is reported as its
 * own outcome. The ordering matters: the child touches nothing shared after setting the
 * flag, so a parent that has seen it set may tear down what the child was using. (D159)
 */

/* Written by the child, read by the parent. Volatile because the compiler cannot see
 * the other thread's write and is entitled to fold the read to its initial value. */
...
/* Roughly a second of spinning on any machine this runs on, and it does not matter if
 * it is ten. It bounds a wait whose alternative is not waiting at all. */
enum { OBS_CHILD_SPIN = 40000000 };

/* Wait for the child to say it finished, without blocking on it. Returns whether it
 * did. */
...
static obs_result check_thread_identities_differ(void) {
    ...
    /* `030-thread/self` asks whether a thread has an identity and
     * `thread-identity-stable` above asks whether it keeps it. Both are satisfied by a
     * function that returns the same constant to everyone - which is a plausible stub,
     * and which makes every mutex and condition variable on the platform silently
     * wrong, since they are all keyed on exactly this value. Two threads is the only
     * way to ask. */
    ...
    ScePthread child = 0;
    int rc =
        scePthreadCreate(&child, NULL, obs_identity_child, NULL, "obscene-identity");
    if (rc != 0) {
...
static void *obs_exclusion_child(void *arg) {
     ...
     * failure this exists to find, and leaving it held would make the parent's destroy
     * the thing that hangs. Recovering matters more than being tidy about a broken
     * lock. */
...
static obs_result check_mutex_excludes_another_thread(void) {
    ...
    /* Mutual exclusion, which is the entire purpose of a mutex and which nothing else
     * in this suite tests. `015-sync/mutex` locks and unlocks on one thread;
     * `mutex-recursion` asks what a second lock from the *same* thread does. Neither
     * can tell a real lock from a function that returns zero, because on one thread
     * both look identical. */
    ...
    ScePthread child = 0;
    int rc =
        scePthreadCreate(&child, NULL, obs_exclusion_child, NULL, "obscene-contend");
    if (rc != 0) {
        (void)scePthreadMutexUnlock(&obs_contended_mutex);
        (void)scePthreadMutexDestroy(&obs_contended_mutex);
    ...
    if (!finished) {
         ...
         * inconclusive result into a crash somewhere unrelated. Leaking one mutex is
         * the cheaper outcome. */
...
static obs_result check_allocations_do_not_overlap(void) {
     ...
     *
     * `direct-memory-round-trip` above releases before allocating again, so it says
     * nothing about this: an allocator that ignores its own bookkeeping and returns the
     * same offset every time passes that check and every value check in `020-memory`.
     * What it produces is two callers writing over each other, and neither of them did
     * anything wrong. */
    const size_t size = 0x4000;
    ...
    obs_report_measure("018-relational/allocations-do-not-overlap",
                       "sceKernelAllocateDirectMemory", "first", (uint64_t)first,
                       "offset");
    obs_report_measure("018-relational/allocations-do-not-overlap",
...
static obs_result check_file_position_tracks_reads(void) {
    ...
    /* Three relations on one descriptor, none of which needs to know the file's
     * contents.
     *
     * A fresh descriptor is at zero; reading N bytes advances the position by exactly
     * the number of bytes the call said it read; and seeking back to that position and
     * reading again yields the same bytes. Together they are the difference between a
     * `read` that works and one that returns a plausible count having done nothing -
     * which every negative check in `040-file` accepts, because they only ever ask it
     * to fail.
     *
    ...
    if (after != (sce_off_t)read_first) {
        (void)sceKernelClose(fd);
        obs_report_measure("018-relational/file-position-tracks-reads",
                           "sceKernelLseek", "position-after-read", (uint64_t)after,
                           "bytes");
    ...
    for (sce_ssize_t i = 0; i < read_first; i++) {
        if (first[i] != again[i]) {
            /* The value is the offset that differed, which is what a reader needs to
             * tell "the second read did nothing" from "the second read got something
             * else". */
...
static const obs_check relational_checks[] = {
    ...
    {"018-relational/event-flag-handles-distinct", "libkernel",
     "sceKernelCreateEventFlag", OBS_CAP_NONE, OBS_CAP_NONE,
     (const void *)&sceKernelCreateEventFlag, check_event_flag_handles_distinct,
     OBS_FROM_SPEC},
    ...
    {"018-relational/direct-memory-round-trip", "libkernel",
     ...
     OBS_FROM_SPEC},
    {"018-relational/semaphore-counts", "libkernel", "sceKernelCreateSema",
     OBS_CAP_NONE, OBS_CAP_NONE, (const void *)&sceKernelCreateSema,
     check_semaphore_count_is_tracked, OBS_FROM_SPEC},
     ...
     *
     * Capabilities are granted in running order, this section runs at 018, and the
     * check that grants `OBS_CAP_FILE` is in `040-file`. So both of these were gated on
     * something that could not exist yet, and **neither has ever run** - on any target,
     * host included. Each reported `skip: a prerequisite capability was not
     * established`, which reads as a platform limitation rather than a build-order
     * mistake, and that is why it survived review in every report it appeared in.
     *
     * The requirement was redundant as well as fatal: each body opens the file itself
     * and skips with a better message when it cannot. `obscene-tool caps` gates the
     * ordering now, so it cannot come back silently. (D158) */
    ...
    /* Per-object state. `spec` for the same reason as the rest: no document states
     * these returns, and "one object's state is not another's" is not this project's
     * opinion. */
    {"018-relational/event-flag-state-is-per-object", "libkernel",
     "sceKernelSetEventFlag", OBS_CAP_NONE, OBS_CAP_NONE,
     (const void *)&sceKernelSetEventFlag, check_event_flag_state_is_per_object,
     OBS_FROM_SPEC},
    ...
    /* `IMPLEMENTATIONS`: shadPS4's handle is a `u32` slot index and PS5PCEM's
     * out-parameter is `?*u32`; fpPS4's is a pointer. Two against one, and the
     * declaration follows the two, which is what makes the third an overrun. (D171) */
     ...
     * threads. `OBS_CAP_THREAD` is granted in `030-thread`, which runs after this
     * section - requiring it here is the exact mistake `obscene-tool caps` now gates
     * (D158). Each body creates its own thread and skips when it cannot, which is a
    ...
    {"018-relational/mutex-excludes-another-thread", "libkernel",
     "scePthreadMutexTrylock", OBS_CAP_NONE, OBS_CAP_NONE,
     (const void *)&scePthreadMutexTrylock, check_mutex_excludes_another_thread,
     OBS_FROM_SPEC},
...
const obs_section obs_section_relational = {
    ...
    "Results compared to each other rather than to an expected value, so no authority "
    "is "
    "needed to check them. Aimed at the vendor surface, where no document supplies "
    "one.",
    relational_checks,
