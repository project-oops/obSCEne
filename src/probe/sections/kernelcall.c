 ...
 * PPSA04263 (Grand Theft Auto V) executes `cd 41` (int 0x41) at image+0x196b91a
 * expecting it to return. We test int $0x41 with selector values (RAX=0, 1) under fault
 * guard protection. If a signal (SIGSEGV, SIGILL, etc.) is raised, the fault guard
 * captures and reports it. If control returns, we record returned RAX, RDX, and RFLAGS.
 */
static obs_result check_int_0x41(void) {
    ...
    for (size_t i = 0; i < OBS_COUNT(selectors); i++) {
        ...
        if (sig == 0) {
            ...
            any_returned = 1;
            last_ret = out_rax;
            obs_report_measure("137-kernelcall/int41-probe", labels[i], "returned", 1u,
                               "bool");
            obs_report_measure("137-kernelcall/int41-probe", labels[i], "rax", out_rax,
                               "hex");
            obs_report_measure("137-kernelcall/int41-probe", labels[i], "rdx", out_rdx,
                               "hex");
            obs_report_measure("137-kernelcall/int41-probe", labels[i], "rflags",
                               out_rflags, "hex");
        } else {
            obs_fault_unregister();
            obs_report_measure("137-kernelcall/int41-probe", labels[i], "returned", 0u,
                               "bool");
            obs_report_measure("137-kernelcall/int41-probe", labels[i], "fault",
    ...
    return obs_partial_value(
        "int 0x41 raised signal; kernel did not return to user space", 0);
#else
