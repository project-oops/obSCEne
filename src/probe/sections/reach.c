 ...
 *
 * obSCEne runs in one of two very different filesystem contexts, and every other
 * section that touches a path reads differently depending on which. As a sandboxed
 * application it sees its own `/app0` mount and its writable `/data`, and little else.
 * As a payload with the exploit's privileges it sees the wider system. Whether a later
 * probe's file access *should* have worked depends entirely on which of those is true,
 * and until now nothing said which.
 *
 * So this asks the question directly: it attempts to open a small set of paths that
 * span the boundary, and reports for each whether the open succeeded. A path inside the
 * app's own jail that opens, and a path in the system namespace that does not, is a
 * process that is sandboxed. The system path opening too is a process that has escaped.
 *
 * # Behaviour, not contents
 *
 * This reads nothing. It opens a path, learns whether the namespace exposes it, and
 * closes it again. The result is the open's own verdict - reachable or not - which is a
 * measurement of *this process's view*, not of anybody's file. Nothing is copied off,
 * nothing is interpreted; the bytes behind these paths never enter the report. That is
 * the line this section is on: it measures what a call does, the way the whole probe
 * does. (selfish#D086)
 */
...
typedef struct {
    const char *path;
    /* 1 if reaching it means the process is outside its own jail, 0 if it is the jail
     * itself. */
    int outside;
    const char *note;
} obs_reach_probe;
...
/* Open O_RDONLY and close at once. Returns 1 if the path was reachable, 0 if not. Reads
 * nothing: the open's success or failure is the entire result. */
...
static obs_result check_reachability(void) {
    ...
    for (unsigned i = 0; i < OBS_COUNT(obs_reach_probes); i++) {
        ...
        /* One record per path: what it is, and whether this process could reach it. The
         * path is a fixed string in this program, not content read off the box. */
    ...
    /* Neither reachable is the host build, or a context with no filesystem at all - not
     * a failure of this code, so it reports what it saw and moves on. */
    obs_report_sysinfo("reach/verdict", "none",
                       "no test path was reachable from this process");
...
static obs_result check_mode_verdict(void) {
    ...
    obs_report_sysinfo("mode/verdict", "unknown",
                       "no GPU interface reached from current namespace");
...
const obs_section obs_section_reach = {
    "047-reach",
    "Sandbox reach",
    "Whether this process is confined to its own mount or can see the system "
    "namespace, from "
