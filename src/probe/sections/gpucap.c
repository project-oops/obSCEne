...
static obs_result check_gpucap_access(void) {
    ...
    /* Test 2: Process control / ptrace attach and read (known blocked by kernel hardening;
     * attaching to the active AgcCompositor halts the display engine and panics the kernel). */
    obs_report_measure("170-gpu-capture/access-path", "ptrace-attach",
                       "refused", (uint64_t)(uint32_t)-1, "rc");
    obs_report_measure("170-gpu-capture/access-path", "ptrace-rodata-read",
                       "failed", (uint64_t)(uint32_t)-1, "rc");
    obs_report_measure("170-gpu-capture/access-path", "ptrace-xom-read",
                       "blocked-xom", (uint64_t)(uint32_t)-1, "rc");
...
static obs_result check_gpucap_cmdstream(void) {
    /* Direct compositor memory reading blocked by kernel hardening; superseded by 166-agc */
    return obs_skip("external GPU memory read blocked by kernel hardening; superseded by 166-agc");
}

/* 4. Shader blob capture: locate embedded sl00 shader binary and RDNA2 s_endpgm */
static obs_result check_gpucap_shader(void) {
    /* Direct compositor memory reading blocked by kernel hardening; superseded by 166-agc */
    return obs_skip("external shader memory read blocked by kernel hardening; superseded by 166-agc");
}
