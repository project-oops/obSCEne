 ...
 * because the census loads a library by name and this one is a library *inside* the kernel
 * module rather than a module of its own - so it reported `this library could not be
 * loaded` and nothing has ever called either function.
 *
 * Meanwhile they are the busiest pair on the platform. Across every guest run the sibling
 * emulator has recorded, **78% of all calls went to the wait** - one function, in one title,
 * through a Unity shim that re-exports it (its D566). Answered with a placeholder it spun
 * 2.27 million times in twelve seconds; modelled on FreeBSD's `_umtx_op(2)` it drops to 26
 * calls (its D573). That model rests on assumptions no run can settle, and they are what
 ...
 * written as the `try` form or not at all cannot be satisfied the usual way - and a probe
 * that hangs loses every check behind it, which this suite has paid for twice.
 *
 ...
 * counter, and calls the *wake*, which cannot block. If the platform never returns from a
 * wait, the worker stays there and the run does not - exactly the arrangement
 * `015-sync/condvar-wakes-a-waiter` already uses for the one other unbounded call in this
 * program, and for the same reason.
 *
 * That also makes the width question safe, which it otherwise would not be. The
 * discriminating case is a word whose high half differs and whose low half matches: under a
 * 64-bit comparison it returns at once, and under a 32-bit one it blocks. **Both outcomes
 * are recoverable** - the blocked one is released by a wake the main thread issues anyway -
 * so the measurement costs nothing whichever way it comes out.
 *
 ...
 * boolean would merge two different faults: a wait that never returned and a worker that
 * died before reaching it are different findings with different causes.
 *
 ...
 * architecture. Stated because it is a real limitation and not a rigorous one - the same
 * caveat `015-sync` carries for the same reason.
 *
 * # No timeout is probed, deliberately
 *
 * `_umtx_op` has a timeout slot and the wrapper may well carry one, but every observed
 * guest call leaves that register zero and nothing establishes its unit. Declaring a third
 * parameter to probe it would be declaring an arity nothing settles, which principle 2
 ...
 * it does not depend on a write landing first. */
static uint64_t s_sa_gate;

/* The word the width question turns on: high half set, low half clear.
 *
 * Waiting on it for a value of zero matches in the low 32 bits and differs in all 64. So a
 * platform comparing 32 bits blocks and one comparing 64 returns at once, and the two are
 * told apart by which happens. This is little-endian reasoning and the target is x86-64:
 * the low half is the one a 32-bit read at this address sees. */
 ...
 * does not work, and the whole section would misreport on a short sleep. */
#define OBS_SA_SETTLE_US 50000u

/* The script. Three waits, in order, each announced by the counter before it is made so a
 ...
 *
 * On the main thread, because a wake cannot block. It is also the question the emulator's
 * model turns on in the other direction: it deliberately does not remember such a wake, on
 * the reasoning that the word carries the state and the wake only ends a sleep. What the
...
static obs_result check_wake_with_no_waiter(void) {
    ...
    /* Nothing is graded. A zero could be success or a count of none, and a non-zero could
     * be an error or a count - all legitimate, and the value is the finding. */
    return obs_pass_value((uint64_t)(uint32_t)rc);
}

/* A word that already differs must not be waited on.
 *
 * This is the defining property of a compare-and-wait, and the one a caller depends on: the
 * wake may have come first, so a wait that slept anyway would be the lost-wakeup hang. It
 * is also the check that starts the worker. */
static obs_result check_wait_returns_on_mismatch(void) {
    ...
    if (state < OBS_SA_MISMATCH_RETURNED) {
         ...
         * the hang this design exists to avoid - and every check below skips. */
    ...
    obs_report_error_code("libkernel_sync_on_address", "sceKernelSyncOnAddressWait",
                          "value already differs", (uint64_t)(uint32_t)s_sa_mismatch_rc);
 ...
 *
 * The round trip, and the only check here that proves the pair works rather than measuring
 * an edge of it. A platform that returns success from both calls while the waiter stays
 * blocked is the exact shape of a stubbed synchronisation layer, and it is silent. */
static obs_result check_wake_releases_a_waiter(void) {
    ...
    /* It has announced the wait but may not be parked in it yet. A wake that arrives first
    ...
    if (s_sa_state < OBS_SA_RELEASED) {
        /* One more, asking for every waiter rather than one, before calling it a failure: a
    ...
    obs_report_measure("032-syncaddr/wake-releases-a-waiter", "sceKernelSyncOnAddressWait",
                       "wait-returned", (uint64_t)(int64_t)s_sa_blocked_rc, "code");
    return obs_pass_value((uint64_t)(uint32_t)rc);
}
...
const obs_section obs_section_syncaddr = {
    ...
    "waited on, whether a wake releases a blocked waiter, and whether the comparison reads "
    "32 bits or 64. Every wait runs on a worker nobody joins, so none of it can hang the "
    "suite.",
    syncaddr_checks,
