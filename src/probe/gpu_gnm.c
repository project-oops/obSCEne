 ...
 * memory, supply a shader binary, build a PM4 command buffer, submit, wait, read back.
 * The signatures and the command-buffer format are only partly public - shadPS4's
 * source has them, but OpenOrbis's headers are mostly stubs for the compute path and no
 * public sample dispatches compute - so much of it would be one source, not the two
 * that let the socket backend be written with confidence. D008 says not to guess at a
 * submission format on a target that took effort to reach.
 *
 * So this refuses, the way `net_target.c` refused before its signatures were confirmed.
 * `obs_gpu_backend_available` returns zero, the GPU section reports a skip with the
 * reason, and nothing announces a capability it cannot honour.
 *
