...
static void emit(const char *s){ unsigned long n=0; while(s[n])n++; sceKernelWrite(1,s,n); }
static void emit_u(unsigned long v){ char b[21]; int i=20; b[i--]=0; if(!v)b[i--]='0'; while(v){b[i--]=(char)('0'+v%10);v/=10;} emit(&b[i+1]); }
static void emit_hex(unsigned long v){ static const char d[]="0123456789abcdef"; char b[17]; for(int i=0;i<16;i++)b[15-i]=d[(v>>(i*4))&0xF]; b[16]=0; emit("0x"); emit(b); }
...
void obs_payload_main(void){
    ...
    emit_u(sceKernelGetTscFrequency()); emit("|hz\n");
    ...
    for(int off=0;off<48;off+=8){
        unsigned long w=0; for(int b=0;b<8;b++) w|=(unsigned long)ver[off+b]<<(b*8);
        emit("OBS|bytes|000-hw/sw-version|sceKernelGetSystemSwVersion|ver|");
        emit_u((unsigned long)off); emit("|"); emit_hex(w); emit("\n");
    }
