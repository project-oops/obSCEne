 ...
 * resolves nothing else. libkernel is callable but not readable from the sandbox, so the
 * export table cannot be walked at runtime - but the vaddrs are known from the real 12.40
 * libkernel_sys.sprx, pulled over FTP and read with selfish (no SDK, real file as oracle):
 *
 *   getpid          vaddr 0x005b0   (word0 runtime confirms base = word0 - 0x5b0)
 *   sceKernelWrite  vaddr 0x16e00
 *
 * So sceKernelWrite_runtime = (word0 - 0x5b0) + 0x16e00. elfldr dup'd the loader socket onto
 * fds 1 and 2, so a write there reaches the sender.
 */
