 ...
 *
 * There is no console here, so this stands in for one: it drives the *encoder* with a synthetic
 * run - a hot function called far past its cap, a cold one, an inlined out-parameter, a buffer
 * too large to inline - writes a real trace stream, and then checks two things:
 *
 *   1. The stream round-trips: every value the encoder put in comes back out of a re-read.
 *   2. The policies that make the payload safe on a real device actually hold - the per-nid
 *      cap limits detail while the count still travels, and an oversized buffer is hashed, never
 *      shipped as bytes.
 *
 * It also runs the on-disk decoder over the same stream so the *parseable* half is exercised for
 * real, not just asserted. Exit non-zero on any failure; that is what the Makefile gates on.
 */
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
...
int main(void) {
    ...
    /* A hot function, called 1000 times. Only the first OBS_TRACE_CAP get detail; the rest just
     * move the counter. This is the mechanism that keeps the payload from drowning the device. */
    uint32_t seq = 0;
    ...
    check(obs_trace_count(&samp, NID_COLD) == COLD_CALLS, "cold nid counted every call");
    ...
    for (uint32_t i = 0; i < buf.head; i++) {
        ...
        if (r->kind == OBS_TRACE_OUTBUF) {
            ...
            if (r->nid == NID_OUT) {
                ...
                small_inline_ok = (r->argc == sizeof small && r->arg[1] == sizeof small &&
    ...
    if (failures == 0) {
        printf("trace_selftest: OK - %u records, hot capped at %u of %u, big buffer hashed\n",
