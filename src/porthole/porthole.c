...
/*
 * THE GO/NO-GO. Reach the hardware video encoder (the VCE block the console uses for
 * its own recordings) and hold an open session.
 *
 * On hardware this drives `libSceVideoRecording` / `libSceVencCore`. obSCEne's
 * `record.c` already calls the *safe* half of that interface (the handle-only calls);
 * what is unproven - and what everything below depends on - is whether an unsigned
 * payload can `Open`/`SetInfo` a session and pull one encoded access unit. That needs
 * those calls' parameter-struct layouts from a lawful source, and it is precisely
 * obSCEne's kind of question: call it, record what came back, grade it by what it ran
 * on.
 *
 * So this returns PORTHOLE_NO_ENCODER until the probe says the door opens. It is not a
 * guess dressed as success (D008); it is the honest "not established yet".
 */
porthole_status porthole_encoder_open(void) {
    return PORTHOLE_NO_ENCODER; /* TODO: gated on the obSCEne encoder-reachability
                                   probe. */
}

/*
 * Capture the composited output and hand back one encoded access unit (Annex-B H.264,
 * as the encoder emits it - no container of ours). `out`/`cap` describe the caller's
 * buffer; `*len` receives how many bytes were written.
 *
 * Depends entirely on porthole_encoder_open having succeeded, and on the display buffer
 * being reachable from an unsigned payload and in a known colour space - the second
 * open question in VIDEO.md. Nothing is captured or encoded here yet.
 */
porthole_status porthole_capture_encode(uint8_t *out, size_t cap, size_t *len) {
    (void)out;
    (void)cap;
    *len = 0;
    return PORTHOLE_UNIMPLEMENTED; /* TODO: capture composited buffer -> hardware encode
                                      -> AU. */
}

/*
 * Apply one decoded controller record to the target's pad state. The record has already
 * passed porthole_pad_decode, so `pad` is trusted here.
 *
 * On hardware this writes the target's own pad structure - button bits and stick range
 * are Ghostpad's, confirmed against real hardware, and a trigger must set BOTH its bit
 * and its pressure byte or the press does not register. Nothing is injected here yet.
 */
porthole_status porthole_pad_apply(const porthole_pad *pad) {
    (void)pad;
    return PORTHOLE_UNIMPLEMENTED; /* TODO: write the target pad structure (Ghostpad
                                      layout). */
}

/*
 * The payload's whole life: open the two listening sockets (9805 out, 9806 in), reach
 * the encoder, then serve encoded frames to whoever connects on 9805 and apply pad
 * records arriving on 9806 until told to stop. Sockets go through the same
 * `obs_net_backend_*` the probe's transport uses; nothing here duplicates a listener.
 *
 * Scaffolded: the loop is described, not run. The moment porthole_encoder_open answers
 * OK, this becomes the few hundred lines VIDEO.md promises, with nothing in porthole.h
 * to change.
 */
porthole_status porthole_run(void) {
    porthole_status encoder = porthole_encoder_open();
    if (encoder != PORTHOLE_OK) {
        return encoder; /* No encoder, no stream. Refuse cleanly rather than serve
                           zeros. */
    }
    /* TODO, once the door is open:
     *   - listen on PORTHOLE_PORT_VIDEO and PORTHOLE_PORT_INPUT
     * (obs_net_backend_listen)
     *   - on a video connection: loop porthole_capture_encode -> write the AU,
     * byte-exact framing
     *   - on an input connection: loop read 24 bytes -> porthole_pad_decode ->
     * porthole_pad_apply, newest-supersedes-oldest per slot by the record's sequence
     * number
     *   - audio is a third socket and the same argument, deliberately after these two
     * work
     */
