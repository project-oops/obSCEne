...
|---|---|---|
| `TARGET` | `prospero` | hardware target: `orbis` (PS4), `neo` (PS4 Pro), `prospero` (PS5), `trinity` (PS5 Pro). Primary target axis. |
| `GEN` | (alias) | backward-compatibility alias for `TARGET` (`GEN=4` selects `orbis`, `GEN=5` selects `prospero`) |
